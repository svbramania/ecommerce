...
| 1 — MVP store | **In progress** | Real product listing/detail pages, full cart/checkout (address, shipping method, promo codes), all live-verified against a temporary test product | Payment/order-completion needs real Stripe test keys (user-provided); a real product catalog doesn't exist yet |
| 2 — Supplier/fulfillment | **In progress** | A real Saleor App + webhook + reference 3PL connector, live-verified up through a real order being placed | Webhook delivery blocked by Saleor's own SSRF filter on the docker-internal address — a one-line, documented, dev-only fix that needs explicit sign-off (flagged by the harness's own safety check, not made silently) |
