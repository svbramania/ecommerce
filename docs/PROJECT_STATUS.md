...
| 2 — Supplier/fulfillment | **Done** (for the reference connector; a second real 3PL is separate work) | Full chain live-verified: order placed → webhook delivered → fulfillment created → tracking set → order `FULFILLED` | A real 3PL's own catalog/API to integrate for real; supplier CSV/SFTP ingestion |
